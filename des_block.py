...
def sbox(sboxNum, sboxInput):
    j = (sboxInput >>1) & 0xf
    i = (sboxInput & 1) | ((sboxInput >> 4) & 0x02)
    lut = (i * 16) + j
    #print "sboxIn is %x" % lut
    return __S_LUT__[sboxNum - 1][lut]
...
class des_block:
	...
	#def ptblock(self, sboxNum):
        #lr = self.ip()
        #r = msg.subblock(32,64)
        #expanded_r = r.e().value()

        #return expanded_r


def ptblock2(pt, sboxNum):
    """plaintext input for each individual sbox, for round1"""
    msg = des_block(pt, 64)
    msg = msg.ip()
    r = msg.subblock(32,64)
    expanded_r = r.e()
    i = sboxNum - 1
    return expanded_r.subblock(6*i,6*i+6).value()

def ptRoundIn(pt, sboxNum):
    """plaintext of round1, relevant to sbox (sboxNum) given the leakage model of Hamming distance between
    round input and round output"""
    msg = des_block(pt, 64)
    msg = msg.ip()
    l = msg.subblock(0 ,32)
    r = msg.subblock(32,64)
    ...
    lp = l.p(-1)
    i = sboxNum - 1
    out = rp.subblock(4*i,4*i+4).value() ^ lp.subblock(4*i,4*i+4).value()

    return out

def roundkey(k, rd):
    """
    Returns the roundkey of the indicated rd number
    """
    key= des_block(k, 64)
    cd = [key.pc1()]
    kshifts= ( 0, 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 )
...
def subkey(rk, sbox):
    """Returns the subkey of the indicated sbox"""
    return int((rk  >> (48 - 6 * sbox)) & 0x3f)
    #return int((rk  >> (6 * (sbox - 1))) & 0x3f)
...
if __name__ == "__main__":
    ...
    for r in range(1,17):
        ...
        print 'round %d: ' %r
        for i in range(1,9):
            print hex(subkey(rk,i))

    for i in range(1,9):
        print hex(ptblock("48656c6c6f202121", i))
